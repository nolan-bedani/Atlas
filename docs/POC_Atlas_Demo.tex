...
départ), et la machine de lab est réparée (\cmd{chaos.sh clean} si un incident cron traîne).
\end{attention}
...
\end{lstlisting}
\begin{montrer}
Le playbook tourne en \textbf{mode simulation} et affiche \cmd{ok=…} \cmd{changed=0} :
l'infrastructure réelle est \textbf{déjà conforme au code}. Aucune dérive.
\end{montrer}
\begin{dire}
...
\subsection*{3.1 — On casse volontairement un service}
\begin{lstlisting}[style=bash]
# (Terminal A) on arrete nginx sur la machine de lab
ansible atltst01p -m command -a "/opt/atlas/scripts/chaos.sh service"
\end{lstlisting}
\begin{montrer}
La commande confirme « INCIDENT DÉCLENCHÉ : service (nginx arrêté) ».
\end{montrer}
\begin{dire}
« Je provoque une panne réelle mais contrôlée. Maintenant, sans rien dire à Zabbix,
retournons voir la supervision. »
\end{dire}

\subsection*{3.2 — Zabbix détecte la panne tout seul}
\begin{lstlisting}[style=bash]
http://localhost:8080     # Monitoring -> Problems (rafraichir)
\end{lstlisting}
\begin{montrer}
Un \textbf{problème apparaît} sur \cmd{atltst01p} (service indisponible). La détection est
automatique — on n'a rien configuré en direct.
\end{montrer}
...
\end{lstlisting}
\begin{montrer}
Retour dans Zabbix : le problème passe en \textbf{Resolved}, l'hôte redevient vert. La
boucle « je casse $\rightarrow$ ça alerte $\rightarrow$ je répare » est bouclée en direct.
\end{montrer}
\begin{attention}
\cmd{chaos.sh} a un garde-fou : il refuse de s'exécuter ailleurs que sur la machine de lab.
Aucun risque pour les vrais services pendant la démo.
\end{attention}
...
\begin{enumerate}[leftmargin=1.5em,itemsep=4pt]
  ...
  \item \textbf{C'est du code} : \cmd{ansible-playbook site.yaml --check --diff --tags zabbix --limit atlzbx01p} $\rightarrow$ 0 changed.
  \item \textbf{Chaos} : \cmd{ansible atltst01p -m command -a ".../chaos.sh service"} $\rightarrow$ Zabbix voit $\rightarrow$ \cmd{... chaos.sh clean} $\rightarrow$ résolu.
